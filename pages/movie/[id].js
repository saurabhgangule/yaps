  ...
  return (
    <>
      <Head>
        ...
        <meta
          name='description'
          content={`${movie?.detail?.overview.slice(0, 160)}...`}
        />
      </Head>
      <SearchBar
      ...
      {movie ? (
        <div>
          {isTorrent ? (
            <TorrentList
              setisTorrent={setisTorrent}
              runtime={movie?.imdb?.imdb?.runtime}
              torrent_list={movie?.imdb?.trb_posts}
            />
          ) : null}
          ...
          {movie?.imdb?.imdb?.video_list[0]?.key ? (
            <div className='text-gray-300'>
              <YouTube
                videoId={movie?.imdb?.imdb?.video_list[0]?.key}
          ...
          <section className='w-full overflow-hidden md:mb-10 lg:overflow-visible'>
            <Heading
            ...
            <section
              ...
              {renderResults(
                sliceArray(
                  movie?.detail?.recommendations?.results || [],
                  movie?.detail?.recommendations?.results.length
                ),
                CardNormal,
                'movie'
              )}
